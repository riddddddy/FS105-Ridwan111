...
const UpdateUser = () => {
  ...
  const fetchUserDetails = async () => {
    try {
      const response = await fetch(`https://fs105-ridwan111.onrender.com/api/users/${user_id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://fs105-ridwan111.onrender.com/api/users/update/${user_id}`, {
        method: 'PATCH',
        headers: {
