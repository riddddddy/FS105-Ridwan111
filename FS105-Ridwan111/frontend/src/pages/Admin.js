...
const Admin = () => {
  ...
  const fetchUsers = async () => {
    try {
        const response = await fetch(`https://fs105-ridwan111.onrender.com/api/users`);
...
const deleteUser = async (user_id) => {
  try {
    await fetch(`https://fs105-ridwan111.onrender.com/api/users/${user_id}`, {
      method: 'DELETE',
    });
  ...
  const saveHandler = async (e) => {
    ...
    const response = await fetch(`https://fs105-ridwan111.onrender.com/api/users/update/${user?.user._id}`, {
      method: 'PATCH',
